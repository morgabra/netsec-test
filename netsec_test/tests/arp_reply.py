...
class ArpReplyTest(base.NetSecTest):
    """
    Test that a valid ARP reply is forwarded.
    """
    ...
    def server(self):
        ...
        if success:
            self.results = {
                "success": True,
                "msg": "received arp reply from host {}".format(src_ip)}
        else:
            self.results = {
                "success": False,
                "msg": "expected 1 arp reply, got {}".format(len(res))}